    ...
    int main() {
        ...
        unsigned char status_carro = 0; // unsigned retira o sinal negativo
        ...
        status_carro |= (1 << BIT_FAROL); // Recebe ele mesmo ou o valor aparte
        status_carro |= (1 << BIT_TRAVA); // (<<) desloca a posição
        ...
        status_carro ^= (1 << BIT_ALARME); // (^) ou exclusivo, altera o estado anterior
        ...
        status_carro &= ~(1 << BIT_TRAVA); // (~) negação ex: 11111011
